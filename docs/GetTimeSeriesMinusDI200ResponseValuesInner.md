# GetTimeSeriesMinusDI200ResponseValuesInner


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**datetime** | **string** | Datetime in local market time for equities and in UTC for forex and cryptocurrencies referring to when the bar with specified interval was opened | [optional] [default to undefined]
**minusDi** | **string** | Minus_di value | [optional] [default to undefined]

## Example

```typescript
import { GetTimeSeriesMinusDI200ResponseValuesInner } from '@twelvedata/twelvedata-node';

const instance: GetTimeSeriesMinusDI200ResponseValuesInner = {
    datetime,
    minusDi,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
